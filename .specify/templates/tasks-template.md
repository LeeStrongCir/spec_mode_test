...
- [ ] T001 [ENV] 选取并确认测试环境（明确环境 URL 和访问凭证）
- [ ] T002 [ENV] 安装执行机必需的工具、语言运行时和测试框架依赖
- [ ] T003 [P] [ENV] 在 `tests/fixtures/mocks/` 中部署 Mock 服务与测试替身（如模拟第三方接口、隔离数据库）
- [ ] T004 [ENV] 准备并注入测试数据集，配置基础 fixtures 和初始状态
