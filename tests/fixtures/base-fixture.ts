{
  "description": "Playwright base fixture for auth E2E tests",
  "selectors": {
    "usernameInput": "[data-testid='username-input']",
    "passwordInput": "[data-testid='password-input']",
    "confirmPasswordInput": "[data-testid='confirm-password-input']",
    "emailInput": "[data-testid='email-input']",
    "loginButton": "[data-testid='login-button']",
    "registerButton": "[data-testid='register-button']",
    "logoutButton": "[data-testid='logout-button']",
    "rememberMeCheckbox": "[data-testid='remember-me-checkbox']",
    "backToLoginLink": "[data-testid='back-to-login-link']",
    "loginError": "[data-testid='login-error']",
    "usernameError": "[data-testid='username-error']",
    "passwordError": "[data-testid='password-error']",
    "confirmPasswordError": "[data-testid='confirm-password-error']",
    "emailError": "[data-testid='email-error']",
    "logoutSuccessMessage": "[data-testid='logout-success-message']",
    "sessionExpiredMessage": "[data-testid='session-expired-message']",
    "consoleSearchInput": "[data-testid='console-search-input']",
    "searchResultItem": "[data-testid='search-result-item']",
    "createHostBtn": "[data-testid='create-host-btn']",
    "hostListTable": "[data-testid='host-list-table']",
    "hostRowPrefix": "host-row-",
    "hostnameInput": "[data-testid='hostname-input']",
    "usernameInputCreds": "[data-testid='credential-username-input']",
    "passwordInputCreds": "[data-testid='credential-password-input']",
    "instanceTypeSelector": "[data-testid='instance-type-selector']",
    "osImageSelector": "[data-testid='os-image-selector']",
    "ipAllocationMode": "[data-testid='ip-allocation-mode']",
    "ipAddressInput": "[data-testid='ip-address-input']",
    "subnetMaskSelect": "[data-testid='subnet-mask-select']",
    "purchaseDuration": "[data-testid='purchase-duration']",
    "costEstimate": "[data-testid='cost-estimate']",
    "buyNowBtn": "[data-testid='buy-now-btn']",
    "confirmDialog": "[data-testid='confirm-dialog']",
    "confirmBtnYes": "[data-testid='confirm-btn-yes']",
    "confirmBtnNo": "[data-testid='confirm-btn-no']",
    "btnStop": "[data-testid='btn-stop']",
    "btnStart": "[data-testid='btn-start']",
    "btnDelete": "[data-testid='btn-delete']",
    "statusLabel": "[data-testid='status-label']",
    "paginationPrev": "[data-testid='pagination-prev']",
    "paginationNext": "[data-testid='pagination-next']",
    "pageTitle": "[data-testid='page-title']"
  },
  "defaultTimeout": 5000,
  "navigationTimeout": 10000,
  "baseUrl": "http://localhost:3000"
}
